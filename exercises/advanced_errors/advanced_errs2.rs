...
impl Error for ParseClimateError {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match *self {
            ParseClimateError::ParseInt(ref e) => Some(e), //Some(ParseIntError),
            _ => None,
        }
    }
}
...
impl FromStr for Climate {
    type Err = ParseClimateError; // use of type: https://doc.rust-lang.org/reference/items/type-aliases.html
...
mod test {
    ...
    fn test_parse_int_bad() {
        ...
        let err = res.unwrap_err();
        if let ParseClimateError::ParseInt(ref inner) = err {
            // use of ref: https://doc.rust-lang.org/std/keyword.ref.html
            assert_eq!(
                err.to_string(),
                format!("error parsing year: {}", inner.to_string())
            );
